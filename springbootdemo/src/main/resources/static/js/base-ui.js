...
(function ($) {
    $.extend({
    	...
    	table: {
			...
			searchByTabId: function(formId,tabId) {
            	var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
    		    var params = $("#" + tabId).bootstrapTable('getOptions');
    		    params.queryParams = function(params) {
                    var search = {};
                    $.each($("#" + currentId).serializeArray(), function(i, field) {
                        search[field.name] = field.value;
    		    ...
    		    $("#" + tabId).bootstrapTable('refresh', params);
    		},
    		// 导出数据
			...
			importPd: function(formId) {
				...
				layer.open({
					...
					btn1: function(index, layero){
						var file = layero.find('#file').val();
						var flag = layero.find('#pdf').val();
						if(flag=='pdf'){
							if (file == '' || !$.common.endWith(file, '.pdf')){
								$.modal.msgWarning("请选择后缀为 “pdf“的文件。");
								return false;
							}
						}else {
							if (file == '' || (!$.common.endWith(file, '.xls') && !$.common.endWith(file, '.xlsx'))){
						...
						var temfile = $('#file')[0].files[0];
						var strnum = 16*1024*1024;
						if(temfile.size>strnum){
							$.modal.msgWarning("上传文件最大不能超过16mb");
							return false;
						}
						var index = layer.load(2, {shade: false});
						$.modal.disable();
			...
			importCda: function(formId) {
				...
				layer.open({
					...
					btn1: function(index, layero){
						...
						if (file == '' || !$.common.endWith(file, '.xml')){
							$.modal.msgWarning("请选择后缀为 “xml“的文件。");
							return false;
						}
						var temfile = $('#file')[0].files[0];
						var strnum = 16*1024*1000;
						if(temfile.size>strnum){
							$.modal.msgWarning("上传文件最大不能超过16mb");
							return false;
						}
			...
			// 通过tab Id 查询表格第col列值
			selectFirstColumnsById: function(id,col) {
				var rows = $.map($('#' + id).bootstrapTable('getSelections'), function (row) {
					return row[$.table._option.columns[col].field];
				});
				if ($.common.isNotEmpty($.table._option.rememberSelected) && $.table._option.rememberSelected) {
					rows = rows.concat(selectionIds);
				}
			...
			// 回显业务字典
			selectCodeValue: function(datas, value) {
				var actions = [];
				$.each(datas, function(index, dict) {
					if (dict.valueCode == ('' + value)) {
						actions.push("<span class='badge'>" + dict.valueName + "</span>");
						return false;
					}
				});
				return actions.join('');
        ...
        operate: {
			...
			contrast: function() {
				var rows = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
				if (rows.length <= 1) {
					$.modal.alertWarning("请至少选择2条记录");
					return;
				};
				var ids = { "ids": rows.join() };
				console.log(ids.ids)
				var url = $.table._option.constratUrl+"?ids="+ids.ids;
				console.log(url)
				createMenuItem(url, "主索引对比");
				//$.operate.submit(url, "post", "json", data);
			},
			...
			approveAll: function() {
				...
				$.modal.confirm("确认要审核选中的" + rows.length + "条数据吗?", function() {
					var url = $.table._option.approveUrl;
					var data = { "ids": rows.join() };
					$.operate.submit(url, "post", "json", data);
				});
			},
			...
			//保存选中
			saveInWindow: function(url, data) {
				var config = {
					url: url,
					type: "post",
					dataType: "json",
					...
					success: function(result) {
						$.operate.successCallbackByCurrentWindow(result);
					}
				};
				$.ajax(config)
			},
			...
			// 成功回调执行事件（父窗体静默更新）
			successCallbackByCurrentWindow: function(result) {
				if (result.code == web_status.SUCCESS) {
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
    	            ...
    	            var $contentWindow = $('.esb_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
    	            $.modal.close();
